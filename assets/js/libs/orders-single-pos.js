...
import { getFirestore, collection, updateDoc ,getDoc,getDocs,query, where,doc  } from "https://www.gstatic.com/firebasejs/9.6.10/firebase-firestore.js";
      ...
      for (const doc of querySnapshot.docs) {
        ...
        orders.items.forEach(item => {
          ...
          row.innerHTML = `
            <td>
              <a href="#" class="text-inherit">
                <div class="d-flex align-items-center">
                  <div>
                    <img src="${item.imageUrl}" alt="" class="icon-shape icon-lg"/>
                  </div>
            ...
            <td><span class="text-body">¢ ${item.price.toFixed(2)}</span></td>
            <td>${item.quantity}</td>
        ...
        subtotalRow.innerHTML = `
          ...
          <td class="fw-medium text-dark">¢ ${orders.totalAmount.toFixed(2)}</td>
        `;
        ordersListTableBody.appendChild(subtotalRow);
  
        const shippingCost = 10.00; // Assuming a fixed shipping cost
  ...
  async function updateOrderStatus(orderId, newStatus) {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const orderDocRef = doc(db, "POSOrders", orderDoc.id);
